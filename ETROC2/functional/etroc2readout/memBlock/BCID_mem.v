...
`ifdef POWER_PINS
module BCID_mem (VDDCE, VDDPE, VSSE, QA, CLKA, CENA, AA, CLKB, CENB, AB, DB, EMAA,
    EMAB, RET1N, COLLDISN);
`else
...
`endif
  ...
  parameter WORDS = 128;
  parameter MUX = 1;
  parameter MEM_WIDTH = 18; // redun block size 1, 8 on left, 10 on right
  parameter MEM_HEIGHT = 128;
  parameter WP_SIZE = 18 ;
...
`endif
  ...
  reg [17:0] data_out;
  reg [17:0] readLatch0;
  reg [17:0] shifted_readLatch0;
  reg [17:0] readLatch1;
  reg [17:0] shifted_readLatch1;
  reg LAST_CLKB;
  reg [17:0] QA_int;
  reg [17:0] writeEnable;
  real previous_CLKA;
  real previous_CLKB;
 ...
 wire  CLKB_;
  ...
  assign `ARM_UD_SEQ QA_ = (RET1N_ | pre_charge_st) ? ((QA_int)) : {18{1'bx}};
...
`endif
  ...
  reg NOT_AB6, NOT_AB5, NOT_AB4, NOT_AB3, NOT_AB2, NOT_AB1, NOT_AB0, NOT_DB17, NOT_DB16;
 ...
 wire  CLKB_;
  ...
  buf B44(DB_[8], DB[8]);
  buf B45(DB_[9], DB[9]);
  buf B46(DB_[10], DB[10]);
  buf B47(DB_[11], DB[11]);
  buf B48(DB_[12], DB[12]);
  buf B49(DB_[13], DB[13]);
  buf B50(DB_[14], DB[14]);
  buf B51(DB_[15], DB[15]);
  buf B52(DB_[16], DB[16]);
  buf B53(DB_[17], DB[17]);
  buf B54(EMAA_[0], EMAA[0]);
  buf B55(EMAA_[1], EMAA[1]);
  buf B56(EMAA_[2], EMAA[2]);
  buf B57(EMAB_[0], EMAB[0]);
  buf B58(EMAB_[1], EMAB[1]);
  buf B59(EMAB_[2], EMAB[2]);
  buf B60(RET1N_, RET1N);
  buf B61(COLLDISN_, COLLDISN);

   `ifdef ARM_FAULT_MODELING
     BCID_mem_error_injection u1(.CLK(CLKA_), .Q_out(QA_), .A(AA_int), .CEN(CENA_int), .Q_in(QA_int));
  `else
  assign QA_ = (RET1N_ | pre_charge_st) ? ((QA_int)) : {18{1'bx}};
  `endif
...
`endif
`timescale 1ns/1ps
module BCID_mem_error_injection (Q_out, Q_in, CLK, A, CEN);
   output [17:0] Q_out;
   input [17:0] Q_in;
   input CLK;
   input [6:0] A;
