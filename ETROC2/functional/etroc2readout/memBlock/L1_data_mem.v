...
//       
//       Creation Date:  Wed Mar 31 21:02:48 2021
//       Version: 	r0p0
//
...
`ifdef POWER_PINS
module L1_data_mem (VDDCE, VDDPE, VSSE, QA, CLKA, CENA, AA, CLKB, CENB, AB, DB, EMAA,
    EMAB, RET1N, COLLDISN);
`else
module L1_data_mem (QA, CLKA, CENA, AA, CLKB, CENB, AB, DB, EMAA, EMAB, RET1N, COLLDISN);
`endif
  ...
  parameter WORDS = 128;
  parameter MUX = 1;
  parameter MEM_WIDTH = 38; // redun block size 1, 18 on left, 20 on right
  parameter MEM_HEIGHT = 128;
  parameter WP_SIZE = 38 ;
...
`endif
  ...
  reg [37:0] data_out;
  reg [37:0] readLatch0;
  reg [37:0] shifted_readLatch0;
  reg [37:0] readLatch1;
  reg [37:0] shifted_readLatch1;
  reg LAST_CLKB;
  reg [37:0] QA_int;
  reg [37:0] writeEnable;
  real previous_CLKA;
  real previous_CLKB;
 ...
 wire  CLKB_;
  ...
  assign `ARM_UD_SEQ QA_ = (RET1N_ | pre_charge_st) ? ((QA_int)) : {38{1'bx}};
...
`endif
  ...
  reg NOT_AB6, NOT_AB5, NOT_AB4, NOT_AB3, NOT_AB2, NOT_AB1, NOT_AB0, NOT_DB37, NOT_DB36;
  reg NOT_DB35, NOT_DB34, NOT_DB33, NOT_DB32, NOT_DB31, NOT_DB30, NOT_DB29, NOT_DB28;
  reg NOT_DB27, NOT_DB26, NOT_DB25, NOT_DB24, NOT_DB23, NOT_DB22, NOT_DB21, NOT_DB20;
  reg NOT_DB19, NOT_DB18, NOT_DB17, NOT_DB16, NOT_DB15, NOT_DB14, NOT_DB13, NOT_DB12;
 ...
 wire  CLKB_;
  ...
  buf B72(DB_[16], DB[16]);
  buf B73(DB_[17], DB[17]);
  buf B74(DB_[18], DB[18]);
  buf B75(DB_[19], DB[19]);
  buf B76(DB_[20], DB[20]);
  buf B77(DB_[21], DB[21]);
  buf B78(DB_[22], DB[22]);
  buf B79(DB_[23], DB[23]);
  buf B80(DB_[24], DB[24]);
  buf B81(DB_[25], DB[25]);
  buf B82(DB_[26], DB[26]);
  buf B83(DB_[27], DB[27]);
  buf B84(DB_[28], DB[28]);
  buf B85(DB_[29], DB[29]);
  buf B86(DB_[30], DB[30]);
  buf B87(DB_[31], DB[31]);
  buf B88(DB_[32], DB[32]);
  buf B89(DB_[33], DB[33]);
  buf B90(DB_[34], DB[34]);
  buf B91(DB_[35], DB[35]);
  buf B92(DB_[36], DB[36]);
  buf B93(DB_[37], DB[37]);
  ...
  buf B101(COLLDISN_, COLLDISN);
   ...
   `ifdef ARM_FAULT_MODELING
     L1_data_mem_error_injection u1(.CLK(CLKA_), .Q_out(QA_), .A(AA_int), .CEN(CENA_int), .Q_in(QA_int));
  `else
  assign QA_ = (RET1N_ | pre_charge_st) ? ((QA_int)) : {38{1'bx}};
  `endif
...
`endif
`timescale 1ns/1ps
module L1_data_mem_error_injection (Q_out, Q_in, CLK, A, CEN);
   output [37:0] Q_out;
   input [37:0] Q_in;
   input CLK;
   input [6:0] A;
...
begin
   ...
   `ifdef ARM_NONREPAIRABLE_FAULT
      `pre_pend_path.SMARCHCHKBVCD_LVISION_MBISTPG_ASSEMBLY_UNDER_TEST_INST.MEM0_MEM_INST.u1.add_fault(7'd2,6'd34,2'd1,2'd0);
   `endif
end
   task add_fault;
