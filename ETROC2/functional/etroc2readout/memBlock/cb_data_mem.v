...
`ifdef POWER_PINS
module cb_data_mem (VDDCE, VDDPE, VSSE, QA, CLKA, CENA, AA, CLKB, CENB, AB, DB, EMAA,
    EMAB, RET1N, COLLDISN);
`else
...
`endif
  ...
  reg [71:0] data_out;
  reg [35:0] readLatch0;
  reg [35:0] shifted_readLatch0;
  reg [35:0] readLatch1;
  reg [35:0] shifted_readLatch1;
  reg LAST_CLKB;
  reg [35:0] QA_int;
 ...
 wire  CLKB_;
  ...
  assign `ARM_UD_SEQ QA_ = (RET1N_ | pre_charge_st) ? ((QA_int)) : {36{1'bx}};
...
`endif
  always @ (EMAA_) begin
  	if(EMAA_ < 0) 
   	$display("Warning: Set Value for EMAA doesn't match Default value 0 in %m at %0t", $time);
  end
  always @ (EMAB_) begin
  	if(EMAB_ < 0) 
   	$display("Warning: Set Value for EMAB doesn't match Default value 0 in %m at %0t", $time);
  end
  ...
  begin
    if (RET1N_int === 1'bx || RET1N_int === 1'bz) begin
      ...
      QA_int = {data_out[70], data_out[68], data_out[66], data_out[64], data_out[62],
  ...
  begin
    if (RET1N_int === 1'bx || RET1N_int === 1'bz) begin
      ...
      if (row_address > 255)
        ...
        row = mem[row_address];
        writeEnable = ~ {36{CENB_int}};
      row_mask =  ( {1'b0, writeEnable[35], 1'b0, writeEnable[34], 1'b0, writeEnable[33],
      ...
      new_data =  ( {1'b0, DB_int[35], 1'b0, DB_int[34], 1'b0, DB_int[33], 1'b0, DB_int[32],
        1'b0, DB_int[31], 1'b0, DB_int[30], 1'b0, DB_int[29], 1'b0, DB_int[28], 1'b0, DB_int[27],
        1'b0, DB_int[26], 1'b0, DB_int[25], 1'b0, DB_int[24], 1'b0, DB_int[23], 1'b0, DB_int[22],
        1'b0, DB_int[21], 1'b0, DB_int[20], 1'b0, DB_int[19], 1'b0, DB_int[18], 1'b0, DB_int[17],
        1'b0, DB_int[16], 1'b0, DB_int[15], 1'b0, DB_int[14], 1'b0, DB_int[13], 1'b0, DB_int[12],
        1'b0, DB_int[11], 1'b0, DB_int[10], 1'b0, DB_int[9], 1'b0, DB_int[8], 1'b0, DB_int[7],
        1'b0, DB_int[6], 1'b0, DB_int[5], 1'b0, DB_int[4], 1'b0, DB_int[3], 1'b0, DB_int[2],
        1'b0, DB_int[1], 1'b0, DB_int[0]} << mux_address);
...
`endif
  ...
  reg NOT_AB1, NOT_AB0, NOT_DB35, NOT_DB34, NOT_DB33, NOT_DB32, NOT_DB31, NOT_DB30;
  ...
  reg NOT_EMAB2, NOT_EMAB1, NOT_EMAB0, NOT_RET1N, NOT_COLLDISN;
 ...
 wire  CLKB_;
  ...
  buf B66(DB_[8], DB[8]);
  buf B67(DB_[9], DB[9]);
  buf B68(DB_[10], DB[10]);
  buf B69(DB_[11], DB[11]);
  buf B70(DB_[12], DB[12]);
  buf B71(DB_[13], DB[13]);
  buf B72(DB_[14], DB[14]);
  buf B73(DB_[15], DB[15]);
  buf B74(DB_[16], DB[16]);
  buf B75(DB_[17], DB[17]);
  buf B76(DB_[18], DB[18]);
  buf B77(DB_[19], DB[19]);
  buf B78(DB_[20], DB[20]);
  buf B79(DB_[21], DB[21]);
  buf B80(DB_[22], DB[22]);
  buf B81(DB_[23], DB[23]);
  buf B82(DB_[24], DB[24]);
  buf B83(DB_[25], DB[25]);
  buf B84(DB_[26], DB[26]);
  buf B85(DB_[27], DB[27]);
  buf B86(DB_[28], DB[28]);
  buf B87(DB_[29], DB[29]);
  buf B88(DB_[30], DB[30]);
  buf B89(DB_[31], DB[31]);
  buf B90(DB_[32], DB[32]);
  buf B91(DB_[33], DB[33]);
  buf B92(DB_[34], DB[34]);
  buf B93(DB_[35], DB[35]);
  buf B94(EMAA_[0], EMAA[0]);
  buf B95(EMAA_[1], EMAA[1]);
  buf B96(EMAA_[2], EMAA[2]);
  buf B97(EMAB_[0], EMAB[0]);
  buf B98(EMAB_[1], EMAB[1]);
  buf B99(EMAB_[2], EMAB[2]);
  buf B100(RET1N_, RET1N);
  buf B101(COLLDISN_, COLLDISN);

   `ifdef ARM_FAULT_MODELING
     cb_data_mem_error_injection u1(.CLK(CLKA_), .Q_out(QA_), .A(AA_int), .CEN(CENA_int), .Q_in(QA_int));
  `else
  assign QA_ = (RET1N_ | pre_charge_st) ? ((QA_int)) : {36{1'bx}};
  `endif
...
`endif
`timescale 1ns/1ps
module cb_data_mem_error_injection (Q_out, Q_in, CLK, A, CEN);
   output [35:0] Q_out;
   input [35:0] Q_in;
   input CLK;
   input [8:0] A;
...
begin
   ...
   `ifdef ARM_NONREPAIRABLE_FAULT
      `pre_pend_path.SMARCHCHKBVCD_LVISION_MBISTPG_ASSEMBLY_UNDER_TEST_INST.MEM0_MEM_INST.u1.add_fault(9'd505,6'd34,2'd1,2'd0);
   `endif
end
   task add_fault;
