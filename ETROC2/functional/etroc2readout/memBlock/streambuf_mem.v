...
//       
//       Creation Date:  Thu Apr  8 13:08:43 2021
//       Version: 	r0p0
//
...
`ifdef POWER_PINS
module streambuf_mem (VDDCE, VDDPE, VSSE, QA, CLKA, CENA, AA, CLKB, CENB, AB, DB, EMAA,
    EMAB, RET1N, COLLDISN);
`else
module streambuf_mem (QA, CLKA, CENA, AA, CLKB, CENB, AB, DB, EMAA, EMAB, RET1N, COLLDISN);
`endif
  ...
  parameter WORDS = 16;
  parameter MUX = 1;
  parameter MEM_WIDTH = 46; // redun block size 1, 22 on left, 24 on right
...
`endif
 ...
 wire  CLKB_;
  ...
  assign `ARM_UD_SEQ QA_ = (RET1N_ | pre_charge_st) ? ((QA_int)) : {46{1'bx}};
...
`endif
  ...
  reg NOT_CENA, NOT_AA3, NOT_AA2, NOT_AA1, NOT_AA0, NOT_CENB, NOT_AB3, NOT_AB2, NOT_AB1;
  reg NOT_AB0, NOT_DB45, NOT_DB44, NOT_DB43, NOT_DB42, NOT_DB41, NOT_DB40, NOT_DB39;
  ...
  reg NOT_EMAA0, NOT_EMAB2, NOT_EMAB1, NOT_EMAB0, NOT_RET1N, NOT_COLLDISN;
 ...
 wire  CLKB_;
  ...
  buf B94(DB_[36], DB[36]);
  buf B95(DB_[37], DB[37]);
  buf B96(DB_[38], DB[38]);
  buf B97(DB_[39], DB[39]);
  buf B98(DB_[40], DB[40]);
  buf B99(DB_[41], DB[41]);
  buf B100(DB_[42], DB[42]);
  buf B101(DB_[43], DB[43]);
  buf B102(DB_[44], DB[44]);
  buf B103(DB_[45], DB[45]);
  buf B104(EMAA_[0], EMAA[0]);
  buf B105(EMAA_[1], EMAA[1]);
  buf B106(EMAA_[2], EMAA[2]);
  buf B107(EMAB_[0], EMAB[0]);
  buf B108(EMAB_[1], EMAB[1]);
  buf B109(EMAB_[2], EMAB[2]);
  buf B110(RET1N_, RET1N);
  buf B111(COLLDISN_, COLLDISN);

   `ifdef ARM_FAULT_MODELING
     streambuf_mem_error_injection u1(.CLK(CLKA_), .Q_out(QA_), .A(AA_int), .CEN(CENA_int), .Q_in(QA_int));
  `else
  assign QA_ = (RET1N_ | pre_charge_st) ? ((QA_int)) : {46{1'bx}};
  `endif
...
`endif
`timescale 1ns/1ps
module streambuf_mem_error_injection (Q_out, Q_in, CLK, A, CEN);
   output [45:0] Q_out;
   input [45:0] Q_in;
   input CLK;
   input [3:0] A;
...
begin
   ...
   `ifdef ARM_NONREPAIRABLE_FAULT
      `pre_pend_path.SMARCHCHKBVCD_LVISION_MBISTPG_ASSEMBLY_UNDER_TEST_INST.MEM0_MEM_INST.u1.add_fault(4'd13,6'd0,2'd1,2'd0);
   `endif
end
   task add_fault;
