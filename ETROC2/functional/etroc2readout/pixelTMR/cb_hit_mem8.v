...
//       
//       Creation Date:  Mon Mar 29 15:16:28 2021
//       Version: 	r0p0
//
...
`ifdef POWER_PINS
module cb_hit_mem8 (VDDCE, VDDPE, VSSE, QA, CLKA, CENA, AA, CLKB, CENB, AB, DB, EMAA,
    EMAB, RET1N, COLLDISN);
`else
module cb_hit_mem8 (QA, CLKA, CENA, AA, CLKB, CENB, AB, DB, EMAA, EMAB, RET1N, COLLDISN);
`endif
// tmrg default do_not_triplicate
  ...
  parameter WORDS = 64;
  parameter MUX = 1;
  parameter MEM_WIDTH = 12; // redun block size 1, 6 on left, 6 on right
  parameter MEM_HEIGHT = 64;
  parameter WP_SIZE = 12 ;
...
`endif
  ...
  reg [11:0] data_out;
  reg [11:0] readLatch0;
  reg [11:0] shifted_readLatch0;
  reg [11:0] readLatch1;
  reg [11:0] shifted_readLatch1;
  reg LAST_CLKB;
  reg [11:0] QA_int;
  reg [11:0] writeEnable;
  real previous_CLKA;
  real previous_CLKB;
 ...
 wire  CLKB_;
  ...
  assign `ARM_UD_SEQ QA_ = (RET1N_ | pre_charge_st) ? ((QA_int)) : {12{1'bx}};
...
`endif
  ...
  reg NOT_CENA, NOT_AA5, NOT_AA4, NOT_AA3, NOT_AA2, NOT_AA1, NOT_AA0, NOT_CENB, NOT_AB5;
  reg NOT_AB4, NOT_AB3, NOT_AB2, NOT_AB1, NOT_AB0, NOT_DB11, NOT_DB10, NOT_DB9, NOT_DB8;
  reg NOT_DB7, NOT_DB6, NOT_DB5, NOT_DB4, NOT_DB3, NOT_DB2, NOT_DB1, NOT_DB0, NOT_EMAA2;
  reg NOT_EMAA1, NOT_EMAA0, NOT_EMAB2, NOT_EMAB1, NOT_EMAB0, NOT_RET1N, NOT_COLLDISN;
 ...
 wire  CLKB_;
  ...
  buf B36(DB_[8], DB[8]);
  buf B37(DB_[9], DB[9]);
  buf B38(DB_[10], DB[10]);
  buf B39(DB_[11], DB[11]);
  ...
  buf B47(COLLDISN_, COLLDISN);
   ...
   `ifdef ARM_FAULT_MODELING
     cb_hit_mem8_error_injection u1(.CLK(CLKA_), .Q_out(QA_), .A(AA_int), .CEN(CENA_int), .Q_in(QA_int));
  `else
  assign QA_ = (RET1N_ | pre_charge_st) ? ((QA_int)) : {12{1'bx}};
  `endif
...
`endif
`timescale 1ns/1ps
module cb_hit_mem8_error_injection (Q_out, Q_in, CLK, A, CEN);
   output [11:0] Q_out;
   input [11:0] Q_in;
   input CLK;
   input [5:0] A;
...
begin
   ...
   `ifdef ARM_NONREPAIRABLE_FAULT
      `pre_pend_path.SMARCHCHKBVCD_LVISION_MBISTPG_ASSEMBLY_UNDER_TEST_INST.MEM0_MEM_INST.u1.add_fault(6'd1,4'd10,2'd1,2'd0);
   `endif
end
   task add_fault;
