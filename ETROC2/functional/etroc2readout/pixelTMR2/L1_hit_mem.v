...
//       
//       Creation Date:  Mon Apr  5 23:31:26 2021
//       Version: 	r0p0
//
...
`ifdef POWER_PINS
module L1_hit_mem (VDDCE, VDDPE, VSSE, QA, CLKA, CENA, AA, CLKB, CENB, AB, DB, EMAA,
    EMAB, RET1N, COLLDISN);
`else
module L1_hit_mem (QA, CLKA, CENA, AA, CLKB, CENB, AB, DB, EMAA, EMAB, RET1N, COLLDISN);
`endif
  ...
  parameter WORDS = 128;
  parameter MUX = 1;
  parameter MEM_WIDTH = 4; // redun block size 1, 2 on left, 2 on right
...
`endif
  ...
  reg NOT_AB6, NOT_AB5, NOT_AB4, NOT_AB3, NOT_AB2, NOT_AB1, NOT_AB0, NOT_DB3, NOT_DB2;
 ...
 wire  CLKB_;
  ...
  buf B24(DB_[2], DB[2]);
  buf B25(DB_[3], DB[3]);
  buf B26(EMAA_[0], EMAA[0]);
  buf B27(EMAA_[1], EMAA[1]);
  buf B28(EMAA_[2], EMAA[2]);
  buf B29(EMAB_[0], EMAB[0]);
  buf B30(EMAB_[1], EMAB[1]);
  buf B31(EMAB_[2], EMAB[2]);
  buf B32(RET1N_, RET1N);
  buf B33(COLLDISN_, COLLDISN);

   `ifdef ARM_FAULT_MODELING
     L1_hit_mem_error_injection u1(.CLK(CLKA_), .Q_out(QA_), .A(AA_int), .CEN(CENA_int), .Q_in(QA_int));
  `else
...
`endif
`timescale 1ns/1ps
module L1_hit_mem_error_injection (Q_out, Q_in, CLK, A, CEN);
   output [3:0] Q_out;
   input [3:0] Q_in;
   input CLK;
   input [6:0] A;
   ...
   reg entry_found;
   reg list_complete;
   reg [13:0] fault_table [127:0];
   reg [13:0] fault_entry;
initial
begin
   `ifdef DUT
   ...
   `ifdef ARM_NONREPAIRABLE_FAULT
      `pre_pend_path.SMARCHCHKBVCD_LVISION_MBISTPG_ASSEMBLY_UNDER_TEST_INST.MEM0_MEM_INST.u1.add_fault(7'd2,2'd2,2'd1,2'd0);
   `endif
end
   task add_fault;
